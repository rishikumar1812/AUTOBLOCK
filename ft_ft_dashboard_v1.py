...
from tray_utils_v1 import SingleInstance, TrayIconManager, hide_console

from ft_config_loader_v1 import (
...
from ft_network_sender_v1 import send_hello
from ft_process_file_v1   import load_stats, _empty_stats
...
class FTDashboard:
    ...
    def _rebuild_ui(self):
        ...
        _row("Stops Today:",   "lbl_stops_today",  COL_WARN)
    ...
    def _update_ui(self, stats: dict):
        ...
        self.lbl_stops_today.config(text=str(stats.get("stops_today", 0)))
        self.lbl_last_data.config(text=stats.get("last_data", "—"))
        self.lbl_updated.config(
            text=datetime.now().strftime("%H:%M:%S"))

        # Blocked Since row — shows the actual time the block started
        # plus how long it's been blocked. Stays populated for as long
        # as the station remains BLOCKED, even through a no-data
        # timeout, since a blocked machine is expected to stop
        # producing new data.
        bmin   = stats.get("blocked_min", 0)
        bsince = stats.get("blocked_since", "—")
        if status == "BLOCKED":
            dur = (f"{bmin//60}h {bmin%60:02d}m"
                   if bmin >= 60 else f"{bmin}min")
            self.lbl_blocked_since.config(
                text=f"{bsince}  ({dur})", fg=COL_BLOCK)
        else:
            self.lbl_blocked_since.config(text="-", fg=COL_MUTED)

        # No-data footer — also shown while BLOCKED, since a blocked
        # station legitimately has no fresh data until it recovers.
        mins = stats.get("minutes_since", 0)
        if mins > 0 and status in ("STOPPED", "BLOCKED"):
